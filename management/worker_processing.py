...
####计算年龄
####(个人编号,(性质，出生日期(yyyy-mm-dd),性别,年份，年度工资))
####(（个人编号,年份），(年龄,离退状态，性别，工作性质，年度工资))
def ageComputed((key, value)):
    try:
        currentDate = value[3]  ###参保日期
        birthDate = value[1][0:4]  ###出生日期
        sex=value[2]
        age = int(currentDate) - int(birthDate)  ##计算住院时年龄
        if (sex == '1'):
            if (age < 60):
                return ((key,value[3]), (age, '0', value[2],value[0],value[4]))
            else:
                return ((key,value[3]), (age, '1', value[2],value[0],value[4]))
        else:
            if (age < 50):
                return ((key,value[3]), (age, '0', value[2],value[0],value[4]))
            else:
...
####对职工参保数据进行初始化
####是否住院过
######((个人编号,年份),(年龄,离退状态，性别，工作性质，年度工资，累计住院天数,住院次数，累计个人支付 20))
######((个人编号),(年份,年龄,离退状态，性别，工作性质，年度工资，累计住院天数,住院次数，累计个人支付 20))
def workerProcessing((key,value)):
    if value[1]==None:
        return (key[0], (key[1],value[0][0], value[0][1], value[0][2],value[0][3],value[0][4],0,0,0))
    else:
        return (key[0], (key[1],value[0][0], value[0][1], value[0][2],value[0][3],value[0][4], value[1][0], value[1][1], value[1][2]))
...
####(个人编号,(性质，出生日期(yyyy-mm-dd),性别,年份，年度工资))
####(（个人编号,年份），(年龄,离退状态，性别，工作性质，年度工资))
worker_charge=charge.join(worker)\
    .map(lambda (key,value):(key,(value[1][0],value[1][1],value[1][2],value[0][0],value[0][1])))\
    .map(ageComputed)\
    .filter(lambda (key,value):isinstance(value,int)==False)\
    .sortByKey()
#
...
#####((个人编号,年份),(年龄,离退状态，性别，工作性质，年度工资，累计住院天数,住院次数，累计个人支付 20))
#####((个人编号),(年份,年龄,离退状态，性别，工作性质，年度工资，累计住院天数,住院次数，累计个人支付 20))
workerHospitalCharge=worker_charge.leftOuterJoin(hospital) \
    .map(workerProcessing)\
    .sortByKey()
...
######(个人编号,(年份，年龄，离退状态,工作性质,性别,年度工资，是否患有慢性病，累计住院天数,住院次数，累计个人支付 20))
out = open('output/worker_processing.csv', 'w+')
